...
status_t LPSPI_MasterTransferNonBlocking(LPSPI_Type *base, lpspi_master_handle_t *handle, lpspi_transfer_t *transfer)
{
    ...
    /*Signal to indicate that the first byte is a command*/
    GPIO_ClearPinsOutput(GPIOB, 1 << 2);

    /*Fill up the TX data in FIFO */
    LPSPI_MasterTransferFillUpTxFifo(base, handle);

    uint8_t u8Index;
    for(u8Index = 0; u8Index < 60; u8Index++){}
    /*Signal to indicate that the first byte is data*/
    GPIO_SetPinsOutput(GPIOB, 1 << 2);
